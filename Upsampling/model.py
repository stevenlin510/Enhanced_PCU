...
class Model(object):
    ...
    def build_model(self):
        ...
        GNet_param_num = np.sum([np.prod(v.shape.as_list()) for v in tf.trainable_variables('generator')])
        logging.info("===number of parameters in generator: {:.4f} K === ".format(float(GNet_param_num / 1e3)))
        if self.opts.use_gan:
            DNet_param_num = np.sum([np.prod(v.shape.as_list()) for v in tf.trainable_variables('discriminator')])
